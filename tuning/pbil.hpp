...
template <typename T> T get_value(const std::vector<int>& gene) {
...
void pbil::update_probabilities(const std::vector<int>& min_gene,
    ...
    for (unsigned b = 0; b < probabilities.size(); ++b) {
        ...
        double lr = (min_gene[b] == max_gene[b] ? learn_rate : learn_rate + neg_learn_rate);
...
void pbil::mutate() {
    ...
    for (auto& p : probabilities) {
        ...
        if (dist(rng) < mutate_prob) {
            ...
            p = p * (1.0 - mutate_shift) + (dist(rng) < 0.5 ? 1.0 : 0) * mutate_shift;
        }
    }
}
...
template <class T, typename... Args> void pbil::optimize(T&& residual, Args&&... args) {
    ...
    auto rf = std::bind(std::forward<T>(residual), _1, std::ref(std::forward<Args>(args))...);
    /*
    Threadpool workers;
    std::vector<thread_data> T;

    for (size_t j = 0, chunk = (samples.size() + 1) / workers.size(); j < workers.size(); j +=
    chunk) { thread_data td; td.start = j; td.end = std::min(td.start + chunk, workers.size() -
    td.start); td.errors(td.end - td.start); T.push_back(td);
    }
    */
    ...
    while (best_err >= etol) {
        ...
        if (start) {
            ...
            if (initial_guess.size() > 0)
                for (int j = 0; j < initial_guess.size(); ++j)
                    samples[0][j] = initial_guess[j];
        }
